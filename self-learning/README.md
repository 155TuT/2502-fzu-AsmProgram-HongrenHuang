...
这是在老师没发 `/teaching` 文件夹前自行按现代规范配置的环境，与教学环境的规范有偏差，因此任何以考试为目的的学习都 **不要** 参照本文件夹
...
可 `cd test` 以尝试使用终端编译并运行其中的程序，也可以重写 `/.vscode/tasks.json` 对当前打开的 *.asm 文件使用 `ctrl+shift+b` 以自动编译并运行，或 `ctrl+shift+p` 以手动选择任务执行
